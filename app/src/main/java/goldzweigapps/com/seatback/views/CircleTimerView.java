package goldzweigapps.com.seatback.views;
...
public class CircleTimerView extends View {
    ...
    private static final float DEFAULT_TIMER_NUMBER_SIZE = 15;
    ...
    // Default color
    private static final int DEFAULT_CIRCLE_COLOR = ColorUtils.parseColor("#4CAF50");
    private static final int DEFAULT_CIRCLE_BUTTON_COLOR = 0xFFFFFFFF;
    private static final int DEFAULT_LINE_COLOR = ColorUtils.BLACK;
    private static final int DEFAULT_HIGHLIGHT_LINE_COLOR = ColorUtils.BLACK;
    private static final int DEFAULT_NUMBER_COLOR = ColorUtils.BLACK;
    private static final int DEFAULT_TIMER_NUMBER_COLOR = ColorUtils.parseColor("#4CAF50");
...
//    private TimerTask timerTask;
    ...
    private void initialize() {
        ...
        mCirclePaintFill = new Paint(Paint.ANTI_ALIAS_FLAG);
        ...
        //color
        mCirclePaintFill.setColor(ColorUtils.RED_500);
        mCirclePaintFill.setStyle(Paint.Style.STROKE);
        mCirclePaintFill.setStrokeWidth(mCircleStrokeWidth);
        // CircleButtonPaint
        ...
        rectF = new RectF();
        period = new Period((long) mCurrentTime);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(rectF, 270, (float) Math.toDegrees(mCurrentRadian), false, mCirclePaintFill);
        canvas.save();
        for (int i = 0; i < 60; i++) {
            canvas.save();
            canvas.rotate(360 / 60 * i, mCx, mCy);
            if (i % 5 == 0) {
                if (360 / 60 * i <= Math.toDegrees(mCurrentRadian)) {
        ...
        float textLength = mNumberPaint.measureText("14");
        canvas.drawText("3", mCx + mRadius + mCircleStrokeWidth / 2 - mGapBetweenCircleAndLine - mLongerLineLength -
                textLength / 2
                - mGapBetweenNumberAndLine, mCy + getFontHeight(mNumberPaint) / 2, mNumberPaint);
        canvas.drawText("6", mCx, getMeasuredHeight() / 2 + mRadius - mCircleStrokeWidth / 2 - mGapBetweenCircleAndLine -
                mLongerLineLength - mGapBetweenNumberAndLine, mNumberPaint);
        canvas.drawText("9", getMeasuredHeight() / 2 - mRadius + mCircleStrokeWidth / 2 + mGapBetweenCircleAndLine +
                        mLongerLineLength + mGapBetweenNumberAndLine + textLength / 2, mCy + getFontHeight(mNumberPaint) / 2,
                mNumberPaint);
        canvas.drawText("12", mCx, getMeasuredHeight() / 2 - mRadius + mCircleStrokeWidth / 2 + mGapBetweenCircleAndLine +
...
//        Log.d(TAG, "onDraw: period " + periodFormatted);
        canvas.drawText(periodFormatted,
                mCx,(int)(getMeasuredHeight() * 0.55), mTimerNumberPaint);
//        canvas.drawText(":", mCx, mCy + getFontHeight(mTimerNumberPaint) / 2, mTimerColonPaint);
        canvas.restore();
        // Timer Text
        canvas.save();
        canvas.drawText(mHintText, getMeasuredWidth() / 2, (int)(getMeasuredHeight() * 0.35), mTimerTextPaint);
        canvas.restore();
        super.onDraw(canvas);
    }
